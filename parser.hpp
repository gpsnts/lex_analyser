...
/**
 ** \file parser.hpp
 ** Define the  LexTrabParser ::parser class.
 */
...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    {
      // command
      char dummy1[sizeof ( LexTrabParser::Command )];
        ...
        switch (this->kind ())
    {
      case symbol_kind::S_command: // command
        value.move<  LexTrabParser::Command  > (std::move (that.value));
        break;
...
#endif
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t,  LexTrabParser::Command && v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const  LexTrabParser::Command & v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      case symbol_kind::S_command: // command
        value.template destroy<  LexTrabParser::Command  > ();
        break;
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
      ...
      {
        YY_ASSERT (tok == token::TOKEN_END || tok == token::TOKEN_YYerror || tok == token::TOKEN_YYUNDEF || tok == token::TOKEN_LEFTPAR || tok == token::TOKEN_RIGHTPAR || tok == token::TOKEN_SEMICOLON || tok == token::TOKEN_COMMA);
      }
#else
    ...
    /// Build a parser object.
     Parser  (LexTrabParser::Scanner &scanner_yyarg, LexTrabParser::Interpreter &driver_yyarg);
    virtual ~ Parser  ();
  ...
  private:
    ...
    // User arguments.
    LexTrabParser::Scanner &scanner;
    LexTrabParser::Interpreter &driver;
    ...
    , location (that.location)
  {
    ...
    {
      case symbol_kind::S_command: // command
        value.copy<  LexTrabParser::Command  > (YY_MOVE (that.value));
        break;
   ...
   Parser ::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    {
      case symbol_kind::S_command: // command
        value.move<  LexTrabParser::Command  > (YY_MOVE (s.value));
        break;
